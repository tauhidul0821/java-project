...
public class TakaJomaFx extends Application {  
	...
	topSeparator.setPadding(new Insets(10, 0, 0, 0));
       ...
       TakaEntryTable = new TableView();
        ...
        DateColumn.setCellValueFactory(new PropertyValueFactory<TakaJoma, String>("Date"));
        DayColumn.setCellValueFactory(new PropertyValueFactory<TakaJoma, String>("Day"));
        TakaColumn.setCellValueFactory(new PropertyValueFactory<TakaJoma, Integer>("Taka"));
	CommentColumn.setCellValueFactory(new PropertyValueFactory<TakaJoma, String>("Comment"));
        
        // Adding the columns
        TakaEntryTable.getColumns().addAll(DateColumn, DayColumn, TakaColumn,CommentColumn);
        
        try{
            TakaJomaService TakaJomaServiceObject = new TakaJomaService();
            // Setting contactList to table as a observable list
            TakaEntryTable.setItems(FXCollections.observableArrayList(TakaJomaServiceObject.getAll()));
        }
        catch(Exception ex){
	...
	bottomSeparator.setPadding(new Insets(5, 0, 0, 0));        
		...
		*/
        ...
        VBox root = new VBox(5,headline,headlineseparator, top, topSeparator, TakaEntryTable,bottomSeparator, addButton);
        root.setPadding(new Insets(10));
		
		//root.getChildren().addAll(TakaEntryTable);
